...
import com.spydevs.fiestonvirtual.framework.mapper.implementations.AddCommentMapper
...
class CommentDataSourceImpl(
    ...
    override suspend fun addRemoteComment(
        addCommentRequest: AddCommentRequest
    ): ResultType<Comment, ErrorResponse> {
        return when (val result = fiestonVirtualApi.addComment(addCommentRequest)) {
            is NetworkResponse.Success -> {
                ResultType.Success(AddCommentMapper.convertFromInitial(result.body))
            }
            is NetworkResponse.ApiError -> {
                ResultType.Error(result.body)
            }
